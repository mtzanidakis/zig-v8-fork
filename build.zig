...
const GnArgs = struct {
    ...
    fn asString(self: GnArgs, b: *std.Build, target: std.Build.ResolvedTarget) ![]const u8 {
        ...
        const is_musl = tag == .linux and target.result.abi == .musl;

        var args: std.ArrayList(u8) = .empty;
        const gpa = b.allocator;

        // musl builds use system ninja; non-musl uses Chrome's siso.
        if (is_musl) {
        ...
        switch (tag) {
            ...
            .linux => {
                if (is_musl) {
                    // On musl (Alpine), use system Clang, headers, and libc++
                    // instead of Chrome's glibc-based toolchain and sysroot.
                    const musl_target = if (arch == .aarch64)
                        "aarch64-alpine-linux-musl"
                    else
                        "x86_64-alpine-linux-musl";
                    try args.appendSlice(gpa, "use_sysroot=false\n");
                    try args.appendSlice(gpa, "clang_base_path=\"/usr\"\n");
                    try args.appendSlice(gpa, "clang_use_chrome_plugins=false\n");
                    try args.appendSlice(gpa, "treat_warnings_as_errors=false\n");
                    try args.appendSlice(gpa, "use_custom_libcxx=false\n");
                    try args.appendSlice(gpa, b.fmt("extra_cflags=[\"--target={s}\"]\n", .{musl_target}));
                    try args.appendSlice(gpa, b.fmt("extra_cxxflags=[\"--target={s}\"]\n", .{musl_target}));
                    try args.appendSlice(gpa, b.fmt("extra_ldflags=[\"--target={s}\"]\n", .{musl_target}));
...
fn bootstrapV8(
    ...
    // Run gclient sync (skip hooks on musl — Chrome's hook scripts
    // download glibc-only binaries that we don't need)
    const gclient_sync = b.addSystemCommand(&.{
        getDepotToolExePath(b, depot_tools_dir, "gclient"),
        "sync",
    });
    if (is_musl) {
        gclient_sync.addArg("--nohooks");
    }
    ...
    if (!is_musl) {
        // Download Chrome's Clang (only needed on glibc where we use
        // Chrome's toolchain instead of the system compiler)
        ...
        create_marker.step.dependOn(&gclient_sync.step);
    }
